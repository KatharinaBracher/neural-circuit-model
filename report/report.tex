...
%delay, mean of stim distr
Problem with experiment simulation design or model.
Change model by learning I in delay.
%no delay
Discarding the delay in the experiment design solves the problem that arises form the length of the delay interval. 
This does not necessarily mean, that the experimental design. When simulating experiments that include a delay period between trial or even between measurement and reproduction, it can be assumed, that information is hold in the system until the next epoch starts. 
...
\section{Influence of External Variability on Behavior}
...
The statistics if stimuli can be summarized by the ratio of the mean and variance of the stimulus range: $\frac{\text{E}}{\text{var}}$.
...
With larger interval times, internal estimates get noisier and regression of reproductions gets stronger (range effect). 
Increasing the mean interval of stimuli, increases the external variability ratio and as a result more weight should be put on the prior ($K$ decreases).
Besides the mean, also the variance of a stimulus distribution influence the reproduction of intervals. 
With larger variance, differences of time intervals is more apparent and discrimination becomes easier. Thus, regression of reproductions gets weaker.
The external variability ratio gets smaller with increasing variance, and the prior can be neglected more. This results in putting more weight on the update ($K$ increases) instead of the prior.
...
For all cases the weight $K$ should increase since variance is increased. %????????????
...
Corresponding slopes in the behavioral analysis all show slopes below zero. Thus, optimal $K$ is predicting the regression effect across multiple ranges. 
There is no significant difference in the slope of behavior in the short and the mid range, but otherwise significant decrease in the slope with increasing mean. The model predicts range effect for large enough changes in the mean (Fig. \ref{fig:new_ranges}b).

Changes in variance are not captured by the model in neither $K$ nor the slope.
Higher variance in the ranges with fewer stimuli shows no significant difference to their counterparts with higher stimuli sampling.
Also, the all range with a larger span of stimuli has no significantly larger $K$ compared to the mid range. 
(Kolmogorov–Smirnov test, n=20, with Bonferroni correction for multiple testing) %???????
...
\begin{figure}[ht]
	...
	\textbf{(b)} Optimal $K$ for a time constant of 165 ms for all ranges in (a) was determined. Simulations were performed 20 times with different seed initialization. $K$ is plotted against the ratio of the mean and the variance of the corresponding stimulus range. Colors correspond to the legend in (a). 
	}
