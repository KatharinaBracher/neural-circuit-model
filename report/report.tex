...
\todo{remove explanation if you have a space contrain}
...
\todo{cites are uncommon in abstracts}
...
\todo{<- make two sentences here}
\todo{provide an example for "time is pervasive"}
...
\todo{Add here an outline section or paragraph}

\todo{For the unexperienced readers: Maybe introduce some more terms lighly: Magnitude, Interval, End of Interval, what is input/output? what is Error signals, regression, reproduciton etc. The introduction should be very easy to understand.}

\todo{Maybe try to merge subsections here and use paragraphs. Also try to give each paragraph a heading.}

\todo{The next to sections sound like: "Related Work"}


\todo{Improve on the headlines. They go too much into the direction of How? Instead of the What?}

\subsection{Behavioral Effects in Magnitude Estimation}
\todo{Magnitude Estimation is not used in the paragraph below.}
...
\todo{Why not? I guess people have tried to. Either forward reference of give a short intuition.}
...
Experiments, in which subjects have to measure and reproduce time intervals, can be used \todo{just: "Time reproduction experiments can be used "} to study how the brain integrates expectations with current sensory input in order to achieve correct timing.
...
Recently, evidence of predictive timing has emerged suggesting, that the CNS \todo{what is CNS?} makes use of predictive signaling mechanisms, allowing the animal to contemplate sensory consequences of actions (\cite{Meirhaeghe2021}, \cite{Egger2019}).
...
This raises the question of how flexible control over brain dynamics can be achieved? \todo{what are brain dynamics?}
...
A brain area that has been associated with timing behavior is the medial prefrontal cortex (MFC) \todo{add a glossary using Latex for these acronyms} (\cite{Wang2018}, \cite{Emmons2017}, \cite{Genovesio2006}, \cite{Lewis2004}, \cite{Shima2000}).
...
\todo{What is population dynamic?}
...
\paragraph{Research Questions}\todo{<- Add a paragraph heading here or create a subsection}
%---> research questions 
...
\todo{Provide an outline again: Nextup Model, then...}
...
The inputs to $u$ and $v$ are governed by a sigmoid activation function $\theta(x) = \frac{1}{1+\text{exp}(-x)}$.\todo{Use text for exp}
...
\todo{Title is not snappy enough imo: "Reproducing Real World Data"}
\todo{Also this should maybe be a subsection.}
...
Biological plausible slopes lie around 0.83 for the short and 0.73 for the long range.\todo{Are the exact values important here? Are they from an experiment, if so which one? Maybe a section about implementing the model and running the experiments is missing somewhere.}
...
\todo{What is Model Regime? Maybe just: "Error Mimization" is enough}
...
The corresponding time constant $\tau$ for the overall minimum of the MSE \todo{use glossary} differ between the two stimulus ranges. For the short stimulus range, $\tau* = 120$ ms and $K* = 11$ yields an overall minimum, whereas $\tau* = 200$ ms, $K* = 20$ yields a overall minimum for the long range. 
...
The simulations were performed with a common time constant $\tau$ that was set between the optimal $\tau*$ \todo{maybe put the * into subscript} for the short and long range, respectively.
...
\section*{Supplements}
\todo{I think you can definitely (and maybe should) include the methods into the main part. Also how you conducted the experiments with the model can go into the main part.}
\setcounter{figure}{0}
\setcounter{table}{0}
