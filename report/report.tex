...
Time reproduction is usually accomplished by at least one presentation of the time interval e.g. between two visual events, and a reproduction period that is terminated by movement initiation from the subject.
...
Recently, evidence of predictive timing has emerged (\cite{Meirhaeghe2021}, \cite{Egger2018}).
...
Thus, flexible motor timing can be achieved by controlling the speed of neural dynamics (\cite{Sohn2019}, \cite{Remington2018}, \cite{Wang2018}).
...
In time reproduction experiments there are two sources of noise: variability of stimuli and internal noise of neural representations.
Previous results have suggested, that the brain uses a speed-control mechanism to anticipate movement initiation time of delayed responses.
Based on this mechanism a circuit model was developed by \cite{Egger2019} and shown to capture classical effects of magnitude estimation when fitted to human data.
...
The model predicted no influence of the change in external variability on the regression effect, but did predict an effect on the error in reproductions.
...
Understanding how temporal statistics are learned and incorporated will help understand computations of timing in the brain. 

% Internal representation of temporal statistic \cite{Acerbi2012}
% Humans have been shown to adapt to the temporal statistics of timing tasks so as to optimize the accuracy of their responses. representations of the experimental distributions up to third-order moments, including variance 


This suggests that adaptation of the dynamics happend to accommodate the new information. 
Further experiments should be done to investigate this switch of taking into account infomration from changed varinace into reprductions. E.G. gradualty changing the variance could reveal a difference between animals when a adaptation is happending. Animals that did not increase perfomrance in the present data might do so if the variance is furher increased. 
This also means that the model would have to be extended to incooperate this adaptaion of the dynamics that change performance.
...
Another finding that simulations of realistic experiments revealed is a limited regime in which the model displays the discussed effects. For stimulus ranges that are much larger or smaller than the ones used, the behavior of the behavior breaks down.
...
%different timing regimes: subsecond, second \cite{Paton2018} 
\cite{Buonomano2002}

%--> experiment suggestion: 
%inclunece of var graduatly, when remapping? more expreme changes, other chages like sampling? how does the discrepencey arises?
