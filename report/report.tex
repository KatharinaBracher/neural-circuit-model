...
Neural trajectories reflect the internal estimate of a time interval.
Thus, flexible motor timing can be achieved by controlling the speed of neural dynamics (\cite{Sohn2019}, \cite{Wang2018}).
Experiments with multiple, consecutive measurement periods before the reproduction showed a sequential updating of the estimate, providing evidence for a predictive process consisting of a simulated motor plan in the measurement epochs (\cite{Egger2019}).
It is suggested, that the population activity reflects an interval-dependent speed command that is updated based on a mismatch between predicted and actual interval in measurement epochs. 
