...
The stimulus range consisted of seven uniformly distributed stimuli from short and long stimulus range 400-700 ms for the short range and 700-1000 ms for long range. The ranges both contained a 700 ms stimulus interval.
%suppl. Fig
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
\subsection{Memory Parameter in Accordance with Error Minimization}
A crucial parameter is the memory parameter $K$, the weight with which the input is adjusted to balance the mismatch between the predicted interval and the stimulus duration. 
We asked whether the observed behavioral effects in real data (Fig. \ref{fig:behavioraleffects}) can be reproduced by the experiment simulation with the circuit model, adjusting $K$. 
Biological plausible slopes lie around 0.83 for the short and 0.73 for the long range (\cite{Thurley2018}, \cite{Jazayeri2010}). Choosing certain values for $K$, model produced plausible slopes for various time constants (Fig. \ref{fig:parameter}a).
Choosing suitable parameters for both ranges, there is a regression to the mean with plausible slopes for both the short and the long range, where K has to be set to a smaller value for the long range, putting less weight on the update compared to the short range.

To find optimal $K$ we minimized the mean squared error, defined as the sum of the squared bias and variance of the mean reproductions (Supplements Eq. \ref{MSE}).
For larger time constants $\tau$ the optimal $K$ increased as well. For all time constants optimal $K$ was smaller for the long range compared to the short range, as already found for $K$ that result in plausible slopes (Fig. \ref{fig:parameter}b). 
In the behavior, the regression to the mean and the range effect, with a stronger regression for the long range is clearly visible (Fig. \ref{fig:parameter}c). 
...
Furthermore, a general underestimation for the large range is observed.
% results in range effect, but general underestimation for large range (probably due to high variance pushes K down)
% optimizing for bias only, K is not consistent anymore, no range effect

\begin{figure}[ht]
	\centering
	\includegraphics{figures/interIparams.pdf}
	\caption{\textbf{Behavioral Effects and Parameter Optimization} 
	\textbf{(a)} Simulations with 500 trials for each pair of memory parameter $K$, and time constant $\tau$ at noise level $\sigma = 0.02$. Simulations were performed with stimuli chosen from the short (left) or long range (right). Color scale represents the slope for each experiment stimulation, which describes the behavioral results. A slope of 1 on the identity line corresponds to perfect reproduction of the stimuli. Behavioral plausible slopes are encircled.
	...
	\textbf{(c)} Mean reproductions for simulation with 500 trials and optimized $K$ for a time constant $\tau = 140$. For the simulation with short stimulus range (black) optimal $K$ was 14, with the long stimulus range (grey) optimal $K$ was 10.5. If the mean lies on the identity line, reproductions corresponds to the stimulus time interval.  
...
% Stimulus List criterions
% Figure seeds, CV
% Figure y, I in experiment for both regimes
...
\begin{figure}[ht]
	...
	\caption{\textbf{Simulation with optimal $K$}
...
% Figure: phase plane, different inputs, realtion I and slope, bsp behavior u, v, y, I
