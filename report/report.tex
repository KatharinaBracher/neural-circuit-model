...
%suppl. Fig
This results in a distribution of reproduced time intervals for each stimulus (Fig. \ref{fig:parameter}b) and a distribution of inputs that drive the reproduction (Fig. \ref{fig:experiment}b).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
\subsection{Update Weight in Accordance with Error Minimization}
We asked, whether the observed behavioral effects in real data (Fig. \ref{fig:behavioraleffects}) can be reproduced by the experiment simulation with the circuit model.

A crucial parameter is the weight $K$, with which the input to the circuit is adjusted to balance the mismatch between the predicted interval and the stimulus duration. 
...
% For what K is the MSE (Supp) minimal? testing for differnt time constants
% Effect larger K for smaller range, smaller K for larger range (putting less weight on the update)
...
%Figure: Optimization, MSE long short, bias, behavior for one tau
% also for high I regime? -> supp

\subsection{Reproducing Behavioral Effects}

\begin{figure}[ht]
	\centering
	\includegraphics{figures/interIparams.pdf}
	\caption{\textbf{Parameter Optimization and Behavioral Effects} 
	\textbf{(a)} 
	\textbf{(b)}
	\textbf{(c)}
	\textbf{(d)}}
