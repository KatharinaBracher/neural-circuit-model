...
In the subsequent sections I describe the circuit model in detail and discuss different input regimes of the model. I then identify model parameters that minimize errors in the behavior and investigate the effects of stimulus statistics on behavior. The predictions of the model are evaluated and compared to data of time reproduction experiments with gerbils and humans.

\section{A Circuit Model for Time Estimation}
The model is build on a basic circuit, which I will introduce at the beginning. I then describe the extension of the circuit to include a mechanism for updating the inputs (\cite{Egger2020}).
...
Because of this, the model yields a repertoire of slopes that can be exploited for time reproduction (\cite{Egger2020}). 
...
To begin with, all three units have a time constant $\tau = 100$~ms and initial conditions of $u, v$ and $y$ are set to $u_0=0.7, v_0=0.2, y_0=0.5$ (Fig. \ref{fig:circuit}a). %have been optimized for in \cite{Egger2020} and
...
\paragraph{The model has three input regimes.}
...
\paragraph{The input to the model is updated for time reproduction.}
At the end of the measurement epoch, an error signal is determined from the difference of $y$ to the threshold $y_{\text{th}}$.
...
\begin{equation} \label{experimentcircuit}
	...
	\end{split}
\end{equation}
The reset mechanism is switched on for one time step after every epoch, which allows the model to reproduce any number of stimulus intervals in succession.
...
If the threshold is crossed before 20~\% of the stimulus interval has passed, the trial is classified as early timeout trial.
...
\paragraph{Procedure of an experiment simulation.}
...
Besides a fixed delay period between each trial, an initial interval at the beginning of the experiment is introduced which allows the model approach the steady-state before the first trial starts (Fig. \ref{fig:epochs}c).
%with the initial values of $u, v, y$ and an initial input $I_0$ . 
...
\todo{rewrite section in a positive way: interestingly, behavioral plausible parameters are very similar to error minimized parameters. Although they do not directly coincide}
...
\paragraph{The intermediate input regime is a better approach to data.}%Comparison of experiment simulations between the intermediate and high input regime
\todo{Motivate why I compare regimes: which one is better suited for simulation. Conclude in the end that intermediate regime is better.}
...
The error in the interval reproductions across the experiment was minimized, with the MSE defined as $\text{MSE} = \text{bias}^2+\text{var}$. The bias was specified as the squared difference between the mean reproduction $\bar{t_{r}}$ for each stimulus interval $t_s$ in the stimulus range $S$ and variance as mean $\sigma^2$ for all stimulus intervals in $S$.
